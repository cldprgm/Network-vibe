...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const authRes = await authApi.get(`/communities/${saveSlug}/`);
        ...
        if (axios.isAxiosError(err) && err.response?.status === 401) {
            ...
            if (newAccessCookie) {
                ...
                try {
                    const retryRes = await retryApi.get(`/communities/${saveSlug}/`);
            ...
            if (logoutCookies && logoutCookies.length > 0) {
                ...
                try {
                    ...
                    const publicRes = await publicApi.get(`/communities/${saveSlug}/`);
