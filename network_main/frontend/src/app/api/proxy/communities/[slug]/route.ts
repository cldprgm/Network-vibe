...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const authRes = await authApi.get(`/communities/${communitySlug}/`);
        return NextResponse.json(authRes.data);
    } catch (err: unknown) {
        if (axios.isAxiosError(err) && err.response?.status === 401) {
            console.log(`Auth GET /communities/${communitySlug}/ 401, refreshing...`);
            const { newAccessCookie, logoutCookies } = await refreshAccessToken(cookieHeader);

            if (newAccessCookie) {
                console.log(`Got new access_token, repeat request on /communities/${communitySlug}/`);
                ...
                try {
                    const retryRes = await retryApi.get(`/communities/${communitySlug}/`);
                    ...
                    return NextResponse.json(
                        { error: 'Failed to fetch community after refresh' },
                        { status: 500 }
                    );
                }
            }
            ...
            if (logoutCookies && logoutCookies.length > 0) {
                ...
                try {
                    ...
                    const publicRes = await publicApi.get(`/communities/${communitySlug}/`);
                    ...
                    return NextResponse.json(
                        { error: 'Failed to fetch public data of the community.' },
                        { status: 500 }
                    );
                }
            }
        ...
        if (axios.isAxiosError(err)) {
            ...
            console.error('CommunityDetail fallback error:', {
                message: err.message,
                status: err.response?.status,
                data: err.response?.data,
            });
        } else {
            console.error('Unknown error during /communities/[slug] request:', err);
        }
        return NextResponse.json({ error: 'Failed to fetch community detail' }, { status: 500 });
    }
}
