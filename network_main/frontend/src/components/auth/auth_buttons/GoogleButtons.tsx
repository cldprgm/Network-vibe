...
import { googleLoginAuth } from '@/services/auth';
...
export default function GoogleButton({ onSuccess, onError, setLoading }: GoogleButtonProps) {
    ...
    const googleLogin = useGoogleLogin({
        ...
        onSuccess: async (codeResponse) => {
            try {
                setLoading(true);
                const { data } = await googleLoginAuth(codeResponse.code);
        ...
        onError: () => {
            onError("Google login popup closed or failed");
            setLoading(false);
        },
    });
